...
namespace SampleFunctionsApp
{
    ...
    {
        public static async Task<List<string>> FindParentAsync(DigitalTwinsClient client, string child, string relname, ILogger log)
        {
            List<string> allTargetIds = new List<string>();
            ...
            try
            {
               // AsyncPageable<IncomingRelationship> rels = client.GetIncomingRelationshipsAsync(child);
                ...
                AsyncPageable<BasicRelationship> allRelationships = client.GetRelationshipsAsync<BasicRelationship>(child);


                await foreach (BasicRelationship relationship in allRelationships)
                {
                    if (relationship.Name == relname)
                        allTargetIds.Add(relationship.TargetId);
